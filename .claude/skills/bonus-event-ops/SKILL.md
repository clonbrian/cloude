...
5. **MPS VIP Point totals in an EX/example line are `turnover ÷ 100`**, for every activity type — verified against all 6 stored items carrying an EX line. The Instant Challenge template's "50" is the lone stale outlier and must not be copied. Note 「bonus」 in Brian's wording means *bonus games*, i.e. the wagered turnover, not the payout. See `html-rules.md`.
